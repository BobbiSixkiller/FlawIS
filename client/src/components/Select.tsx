"use client";
import Icon from "@/components/Icon";
import { cn } from "@/lib/utilsClient";
import {
  Listbox,
  ListboxButton,
  ListboxOption,
  ListboxOptions,
} from "@headlessui/react";
import type { AriaAttributes, Ref } from "react";
interface Option<T> {
  name: string;
  value: T;
}
export interface SelectProps<T> extends AriaAttributes {
  disabled?: boolean;
  placeholder?: string;
  name?: string;
  id?: string;
  options: Option<T>[];
  multiple?: boolean;
  value?: T | T[] | null;
  onChange: (value: T | T[]) => void;
  onBlur?: () => void;
  ref?: Ref<HTMLButtonElement>;
}
export default function Select<T>({
  placeholder,
  name,
  options,
  disabled = false,
  multiple = false,
  value,
  onChange,
  ref,
  ...props
}: SelectProps<T>) {
  const selected = Array.isArray(value)
    ? options.filter((o) => value.includes(o.value))
    : options.find((o) => o.value === value);
  return (
    <Listbox
      value={selected ?? (multiple ? [] : undefined)}
      disabled={disabled}
      name={name}
      onChange={(val: Option<T> | Option<T>[]) =>
        Array.isArray(val)
          ? onChange(val.map((v) => v.value))
          : onChange(val.value)
      }
      multiple={multiple}
      by="value"
    >
      <ListboxButton
        {...props}
        ref={ref}
        className={cn([
          "h-9 py-1 group flex items-center justify-between px-3 outline-hidden disabled:bg-slate-50 disabled:ring-slate-200 disabled:shadow-none shadow-xs rounded-md w-full ring-1 ring-inset ring-gray-300 focus:ring-2 sm:text-sm/6",
          "dark:bg-gray-800 dark:ring-gray-600 dark:disabled:bg-gray-900 dark:disabled:ring-gray-700 dark:disabled:text-slate-500 dark:text-white/85",
          multiple ? "" : "py-1.5",
          props["aria-invalid"]
            ? "ring-red-500 focus:ring-red-500 dark:ring-red-500 dark:focus:ring-red-500"
            : "focus:ring-primary-500 dark:focus:ring-primary-300",
        ])}
      >
        <span
          className={`line-clamp-1 ${
            selected && !disabled
              ? "text-gray-900 dark:text-white"
              : "text-gray-500"
          }`}
        >
          {selected
            ? Array.isArray(selected)
              ? selected.map((i) => i.name).join(", ")
              : selected?.name
            : placeholder}
        </span>

        <Icon
          name="chevron-down"
          className="size-3 group-data-open:rotate-180 text-gray-300"
        />
      </ListboxButton>
      <ListboxOptions
        anchor="bottom"
        transition
        className={cn([
          "origin-top transition duration-200 ease-out data-closed:scale-95 data-closed:opacity-0 sm:text-sm/6 outline-hidden z-50 w-(--button-width) [--anchor-gap:4px] sm:[--anchor-gap:8px] p-2 rounded-lg shadow-lg text-gray-900 bg-white ring-1 ring-gray-300",
          "dark:bg-gray-800 dark:text-white dark:ring-gray-800",
          props["aria-invalid"] ? "ring-red-500" : "",
        ])}
      >
        {options.map((option, i) => (
          <ListboxOption
            key={i}
            value={option}
            className={cn([
              "data-focus:bg-primary-500 data-focus:text-white data-selected:font-semibold cursor-pointer -mx-2 px-2 group flex gap-2 items-center justify-between",
              "dark:data-focus:bg-primary-300",
            ])}
          >
            {option.name}
            <Icon
              name="check"
              className="size-3 stroke-2 hidden group-data-selected:block"
            />
          </ListboxOption>
        ))}
      </ListboxOptions>
    </Listbox>
  );
}
