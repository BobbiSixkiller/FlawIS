...
export default function TabMenu({ tabs }: { tabs: TabProps[] }) {
  ...
  return (
    <div className="h-20">
        ...
        {tabs.map((tab, i) => {
          ...
          return (
            <div key={i} className="h-14 flex items-center relative">
              <Link
                href={tab.href}
                className={`px-4 py-1 rounded-md text-gray-900 dark:text-white/85 hover:bg-gray-700/10 dark:hover:bg-gray-600/10 inline-flex items-center gap-2 ${
      ...
      <div className="h-px w-full bg-gray-300 dark:bg-gray-600 translate-y-[-2px]" />
    </div>
  );
}
