...
function EditMember(props) {
	...
	const { handleSubmit, handleChange, handleBlur, values, setValues, errors, valid, isSubmitting } = useFormValidation(INITIAL_STATE, validateMember, editMember);
	...
	return(
		<Form onSubmit={handleSubmit}>
			<ModalHeader toggle={() => setModal(!modal)}>Upraviť riešiteľa</ModalHeader>
			<ModalBody>
		        ...
		        <Col sm={4}>
		      	...
		      	<Row form className="justify-content-center">
					<Col sm={8}>
						<CustomInput type="radio" id="basic" name="role" value="basic" label="Riešiteľ" inline defaultChecked={values.role === "basic" ? true : false} onChange={handleChange} />
		            	<CustomInput type="radio" id="deputy" name="role" value="deputy" label="Zástupca" inline defaultChecked={values.role === "deputy" ? true : false} onChange={handleChange} />
		            	<CustomInput type="radio" id="leader" name="role" value="leader" label="Hlavný" inline defaultChecked={values.role === "leader" ? true : false} onChange={handleChange} />
					</Col>
					<Col sm={4}>
						<FormGroup>
							<CustomInput type="switch" id="active" name="active" label="Active"  defaultChecked={values.active} onChange={handleActiveChange} />
						</FormGroup>  
					</Col>
				</Row>
				{backendMsg && 
