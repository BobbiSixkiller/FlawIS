				...
				<Container className="my-5">
					{"_id" in userGrants[0] ? (
									...
									{userGrants
										...
										.map((grant, index) => {
											return grant.budget
												.filter(
													(budget) =>
														new Date(budget.year).getFullYear() == year
												)
												.map((budget) => (
													<tr key={index}>
														<td>{index}</td>
														...
														{location.pathname.includes("/mygrants") ? (
															<td>
																<Button
																	onClick={() => {
																		history.push("/mygrants/" + grant._id);
																	}}
																	color="info"
																>
																	Detail
						...
						<Alert className="text-center" color="info">
							Používateľ nemá pridelené žiadne granty!
						</Alert>
					)}
				</Container>
			</Fade>
