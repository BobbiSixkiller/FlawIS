...
function EditUser(props) {
  ...
  const { handleSubmit, handleChange, handleBlur, values, errors, valid, isSubmitting } = useFormValidation(INITIAL_STATE, validateUser, Update);
        ...
        {user.role === "admin" &&
          <FormGroup row className="justify-content-center">
            <Col sm={6}>
              ...
              <Input type="select" name="role" id="role" value={values.role} onChange={handleChange}>
        ...
        <FormGroup row className="justify-content-center">
          <Col sm={6}>
            <Row form className="justify-content-between">
              <Button className="ml-1" outline color="primary" onClick={() => history.goBack()}>Späť</Button>
              <Button outline color="warning" onClick={() => setIsOpen(!isOpen)}>Zmena hesla</Button>
              <Button className="mr-1" color="success" disabled={isSubmitting} type="submit">Upraviť</Button>
            </Row>
          </Col>
        </FormGroup>
      </Form>
