...
import { Fade, Form, FormGroup, FormFeedback, Col, Alert, Button, Input, Label } from "reactstrap";
...
function PasswordRedet() {
	...
	const resetPassword = async () => {
		...
		try {
	      	...
	      	err.response.data.error.message && setBackendError(err.response.data.error.message);
	      	setLoading(false);
	    }
	}

	const { handleSubmit, handleChange, handleBlur, values, errors, valid, isSubmitting } = useFormValidation(INITIAL_STATE, validatePasswordReset, resetPassword);
		...
		<Fade>
	      ...
	      <Form className="my-3" onSubmit={handleSubmit}>
	        ...
	        {backendMsg && 
	          <FormGroup row className="justify-content-center">
	            <Col sm={6}>
	              <Alert color="success">{backendMsg}<Button close onClick={() => setBackendError(null)} /></Alert>
	            </Col>
	          </FormGroup>
	        }
	        {backendError && 
