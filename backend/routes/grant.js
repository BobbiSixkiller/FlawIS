...
router.get('/', verify, async (req, res) => {
	...
	if (user.role === "admin" || user.role === "supervisor") {
		try {
			const grants = await Grant.find().sort({updatedAt: -1}).populate('budget.members.member').populate({path: 'announcements', populate: {path: 'issuedBy'}});
...
router.post('/:grant_id/addBudget', verify, async (req, res) => {
	...
	if (user.role === "admin" || user.role === "supervisor") {
		...
		if(grant.budget.some(budget => new Date(budget.year).getFullYear() === new Date(req.body.year).getFullYear())) {
			return res.status(400).send({error: "Pre daný rok môže byť vytvorený iba jeden rozpočet!"});
		}
...
router.post('/:grant_id/budget/:budget_id/addMember', verify, async (req, res) => {
	const user = req.user[0];
	if (user.role === "admin" || user.role === "supervisor") {
		const {error} = await membersValidation(req.body);
		if (error) return res.status(400).send({error: error.details[0].message});

		const grant = await Grant.findOne({_id: req.params.grant_id, "budget._id": req.params.budget_id});
		if (!grant) return res.status(404).send({error: "Grant alebo rozpočet nebol nájdený"});
...
router.post('/:grant_id/file', verify, upload.array('files', 5), async (req, res) => {
	const url = req.protocol + '://' + req.get('host');
	const user = req.user[0];

	if (user.role === "admin" || user.role === "supervisor") {
		if (req.files.length === 0) return res.status(400).send({error: "Neboli zaslané žiadne súbory!"});
...
router.get('/:id', verify, async (req, res) => {
	...
	if (user) {
		try {
			const match = await Grant.findById(mongoose.Types.ObjectId(req.params.id)).populate('budget.members.member').populate({path: 'announcements', populate: {path: 'issuedBy'}});
			if (match === null) return res.status(404).send({error: "Grant nebol nájdený!"});
...
router.put('/:id', verify, async (req, res) => {
	...
	if (user.role === "admin" || user.role === "supervisor") {
		...
		try {
			const update = await Grant.findOneAndUpdate({_id: req.params.id}, {$set: req.body}, {new: true});	
			if (update === null) return res.status(404).send({error: "Grant nebol nájdený!"});
...
router.put('/:grant_id/budget/:budget_id', verify, async (req, res) => {
	...
	if (user.role === "admin" || user.role === "supervisor") {
		...
		try {
			let grant = await Grant.findOne({_id: req.params.grant_id, "budget._id": req.params.budget_id });
			if (!grant) return res.status(404).send({error: "Grant alebo rozpočet nebol nájdený"});
			...
			if (result === null) return res.status(404).send({error: "Grant nebol nájdený!"});
...
router.put('/:grant_id/budget/:budget_id/member/:member_id', verify, async (req, res) => {
	const user = req.user[0];
	if (user.role === "admin" || user.role === "supervisor") {
		const {error} = await membersValidation(req.body);
		if (error) return res.status(400).send({error: error.details[0].message});
		...
		try {
			const result = await Grant.findOneAndUpdate(
				{ _id: req.params.grant_id, "budget._id": req.params.budget_id, "budget.members._id": req.params.member_id },
		        {
		          $set: {
		        ...
		        {
		          arrayFilters: [{ "budget._id": req.params.budget_id }, { "member._id": req.params.member_id }],
		          new: true
		        }
			);
			...
			if (!req.body.active) {
				...
				if (!result2) return res.status(404).send({error: "Grant, rozpočet alebo riešiteľ nebol nájdený!"});
				return res.status(200).send({msg: "Riešiteľ bol aktualizovaný."});
			}
			if (!result) return res.status(404).send({error: "Grant, rozpočet alebo riešiteľ nebol nájdený!"});
			return res.status(200).send({msg: "Riešiteľ bol aktualizovaný."});
		} catch(err) {
			res.status(500).send({error: err.message});
		}
...
router.delete('/:grant_id/budget/:budget_id/member/:member_id', verify, async (req, res) => {
	...
	if (user.role === "admin" || user.role === "supervisor") {
		...
		try {
			await grant.budget.id(req.params.budget_id).members.id(req.params.member_id).remove();
			const updatedGrant = await grant.save();
